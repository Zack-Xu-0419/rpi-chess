...
def getBoardState(output, edges=[0, 0, 0, 0]):
    ...
    upper_green = np.array([127, 255, 255])
    ...
    circles = cv.HoughCircles(edges, cv.HOUGH_GRADIENT, 1,
                              minRadius=8, maxRadius=30, param2=14, minDist=25)
    ...
    for j in range(8):
        ...
        for i in range(8):
            ...
            if not isSomething:
                ...
                curr = gray_orig[int(j * dify + consty + dify/2-20): int((j+1) * dify+consty - dify/2+20), int(const+i *
                                 difx + difx/2-20):int(const+(i+1) * difx-difx/2+20)]
                avg = np.mean(curr)
                print(avg)
                if avg < 110:
                    row.append(2)
